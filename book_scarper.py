...
books_dict = {}
authors = []  # List to store unique authors
    ...
    for book_div in soup.find_all('div', class_='elementList'):
        ...
        if title_tag and author_tag:
            ...
            if info_tag:
                ...
                for part in parts:
                    if 'avg rating' in part.lower():
                        avg_rating = part.split('avg rating')[-1].strip()
                    elif 'published' in part.lower():
            ...
            if author not in authors:  # Add author to the list only if not already present
